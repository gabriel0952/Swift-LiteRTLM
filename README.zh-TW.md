...
- 多輪對話 API，支援系統指令與串流輸出
- 低階 Session API，適用於進階場景
- 內建模型下載器，支援進度追蹤、暫停與續傳
- 多模態輸入：圖片與音訊搭配文字
- 工具呼叫（Function Calling），以 JSON Schema 定義工具
- 圖片前處理，推論前自動縮放相機照片
- Swift 6 並發，actor 執行緒安全、async/await
- 支援 CPU、GPU（Metal）與 NPU 後端

## 系統需求

- iOS 17+、Xcode 16+、Swift 6.0
- `.litertlm` 模型檔案（例如 [Gemma 4 E2B](https://huggingface.co/litert-community/gemma-4-E2B-it-litert-lm)）
- 預建的 `LiteRTLM.xcframework`，從 [GitHub Releases](https://github.com/gabriel0952/Swift-LiteRTLM/releases) 下載後放置於 `Frameworks/LiteRTLM.xcframework`
...
下載器支援暫停與續傳，續傳資料會自動保存到磁碟。SwiftUI 整合方式請參閱[下載進度追蹤](#下載進度追蹤)。
...
下載在前景執行，App 被暫停時下載會停止。續傳資料會保存到磁碟，再次呼叫 `download()` 即可從中斷處繼續。
...
以 JSON Schema 定義工具，搭配 execute closure：
...
當 `automaticToolCalling` 為 `false` 時，工具呼叫會以 `reply.toolCalls` 回傳，由你手動執行。

## 圖片前處理

相機照片通常高達 4000×3000+ 像素，遠超端側 LLM 所需。使用 `ImageProcessor` 在推論前縮放圖片：
...
let config = EngineConfig(
    modelPath: "/path/to/model.litertlm",
    backend: .gpu,
    visionBackend: .gpu,       // nil = 停用
    audioBackend: .cpu(),      // nil = 停用
    maxNumTokens: 4096,        // KV-cache token 上限（nil = 模型預設值）
    maxNumImages: 4,           // 每輪最多圖片數量（nil = 模型預設值）
...
注意：`generateContent` 與 `generateContentStream` 可正常使用，但 `runPrefill`、`runDecode`、`Session.cancelProcess()` 目前會拋出 `LiteRtLmError.unsupportedFeature`，因為這些控制點尚未由 upstream public C API 暴露。
...
注意：由於官方 LiteRT-LM public C API 只公開 info/warning/error/fatal，`.verbose` 與 `.debug` 目前會映射到 native 的 `info` 閾值。

## 錯誤處理

所有錯誤都遵循 `LocalizedError`，`error.localizedDescription` 會回傳適合在 UI 中顯示的可讀訊息。
...
| 方法 / 屬性 | 說明 |
|-------------|------|
| `init(config: EngineConfig)` | 建立 Engine |
...
| `isInitialized` | `status == .ready` 時為 `true` |
...
| `download(from:)` | 下載模型，預設從 HuggingFace 下載 Gemma 4 E2B |
| `pause()` | 暫停下載（續傳資料會保存到磁碟） |
| `cancel()` | 取消下載並捨棄續傳資料 |
| `deleteModel()` | 刪除已下載的模型檔案 |
| `status` | 目前下載狀態 |
| `progress` | 0.0 至 1.0 |
| `isDownloaded` | 模型檔案是否存在於磁碟 |
| `localFileURL` | 模型檔案的完整路徑 |
| `modelFilePath` | 方便傳入 `EngineConfig(modelPath:)` |
...
| `cacheDir` | 快取目錄，`nil` = 模型所在目錄，`":nocache"` = 停用 |
| `recommendedCacheDirectory` | （static）`~/Library/Caches/LiteRTLM/` |

## 建構 XCFramework

從 [GitHub Releases](https://github.com/gabriel0952/Swift-LiteRTLM/releases) 下載預建的 `LiteRTLM.xcframework`，放置於 `Frameworks/LiteRTLM.xcframework`。如需從原始碼重建（例如更新至新版 upstream）：
...
- iOS 模擬器不支援 GPU 後端，測試時請使用 `.cpu()`。Session 串流在模擬器上會回退為同步生成。
